...
class AssignPosition extends Component {
  ...
  createAssign = (val, val2) => {
    ...
    if (!check2) {
      ...
      this.setState({
        validAssign:
          'Please select an assignment period to replace or select Add New'
        //selectedAssignStart,
        //selectedAssignEnd
      })
    ...
    if (!check3) {
      this.setState({
        validAssignLength:
          'By selecting Add New you will exceed the number of assignments for one staff.'
      })
...
debugger;
  ...
  render() {
    let mplresult = this.props.allRoles
      ...
      .map(x => x.positions)
      .reduce((prev, x) => prev.concat(x), [])
      .filter(x => x.mplID == this.props.selectedMplID)
                  ...
                  <tbody>
                    <tr>
                      <td ColSpan="2">
                        ...
                        />
                        <b className="card-text text-danger">
                          {this.state.validAssign}
                        </b>
                        <b className="card-text text-danger">
                          {this.state.validAssignLength}
                        </b>
                      </td>
                    </tr>
                  </tbody>

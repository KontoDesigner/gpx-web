...
class Table extends Component {
    ...
    rowRenderer = ({ columns, index, className, style, ...props }) => {
        const rowClassName = (this.state.list.length < 8 ? ' hidden-scroll ' : '') + className + (index % 2 === 0 ? ' even' : ' odd') + (1 === 1 ? ' selected' : '');
    ...
    _sort = ({ sortBy, sortDirection }) => {
        ...
        if (prevSortDirection === sortDirection.DESC) {
         ...
         const ola = Object.assign([], this.state.list);
        //const ola = { ...this.state.list }

        const list = ola.sort(function (a, b) {
            return a.firstName - b.firstName;
        });

        // const list = ola
        //     .sortBy(item => item[sortBy])list
        //     .update(
        //         unsorted =>
        //             sortDirection === sortDirection.DESC ? unsorted.reverse() : unsorted
        //     );

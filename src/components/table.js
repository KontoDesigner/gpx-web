...
const Table = props => {
  const cellRenderer = function(params) {
    if (params.column && params.column.colDef) {
      if (params.column.colDef.type === 'icon') {
        ...
        if (params.column.colDef.title && params.column.colDef.title.length > 0) {
          return `<i title=${params.column.title} class="${icon}" />`
        }
      ...
      if (
        params.column.colDef.type === 'datetime' &&
        params.column.colDef.format
      ) {
  ...
  const defaultMaxTableHeight = props.maxTableHeight
    ? props.maxTableHeight
    : 550
  const tableHeight =
    height > defaultMaxTableHeight ? defaultMaxTableHeight : height
